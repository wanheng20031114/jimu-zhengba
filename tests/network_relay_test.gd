...
func _run() -> void:
	...
	_check("new computer starts at normal difficulty", clients[0].room.slots[1].bot_difficulty == "normal")
	for difficulty: String in Protocol.BOT_DIFFICULTIES:
		clients[0].configure_slot(1, "bot", 1, difficulty)
		await _until(func(): return clients[0].room.slots[1].bot_difficulty == difficulty)
		_check("computer difficulty roundtrip " + difficulty, clients[0].room.slots[1].bot_difficulty == difficulty)
	var invalid_before: int = errors.size()
	clients[0].configure_slot(1, "bot", 1, "impossible")
	await _until(func(): return errors.size() > invalid_before)
	_check("relay rejects unknown difficulty without changing computer", errors.size() > invalid_before and errors[-1].code == "invalid_difficulty" and clients[0].room.slots[1].bot_difficulty == "nightmare")
	invalid_before = errors.size()
	clients[0].configure_slot(0, "human", 0, "hard")
	await _until(func(): return errors.size() > invalid_before)
	_check("relay rejects human gathering cheats", errors.size() > invalid_before and errors[-1].code == "invalid_difficulty" and clients[0].room.slots[0].bot_difficulty == "normal")
	clients[0].start_match()
	...
	_check("start config preserves nightmare difficulty and unique computer name", clients[0]._match.players[1].bot_difficulty == "nightmare" and clients[0]._match.players[1].name == "电脑 2")
