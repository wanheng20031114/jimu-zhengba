...
func _run() -> void:
	...
	for key: Key in [KEY_Q, KEY_W, KEY_E]:
		await _key(key)
	check(barracks.production.training.map(func(job): return job.kind) == ["swordsman", "shield_guard", "spearman"], "physical QWE creates the three visible barracks jobs in order")
	var before_page_gold: int=player.gold
	await _key(KEY_Y)
	check(hud._action_page==1 and hud._actions.map(func(action): return action.id)==["war_elephant","light_cavalry",""],"Y exposes second-page cavalry and return action")
	check(player.gold==before_page_gold and barracks.production.training.size()==3,"page switch never spends gold or trains a unit")
	await _key(KEY_R)
	check(barracks.production.training.size()==3,"hidden second-page slot cannot dispatch a stale first-page order")
	await _key(KEY_W)
	check(barracks.production.training.back().kind=="light_cavalry" and player.gold==before_page_gold-70,"physical W recruits the displayed light cavalry on page two")
	hud._on_queue_cancel(3)
	game.command_bus.tick()
	hud.refresh()
	check(barracks.production.training.size()==3 and player.gold==before_page_gold,"second-page unit cancels through the actual queue with full refund")
	await _key(KEY_E)
	check(hud._action_page==0 and hud._actions[0].id=="swordsman","displayed E return control restores the first page")
	await _key(KEY_Y)
	_select([factory])
	check(hud._action_page==0,"switching producer resets the shared action page")
	await _key(KEY_Q)
	await _key(KEY_W)
	...
	await _key(KEY_Q)
	await _key(KEY_Q)
	check(hud._actions.map(func(action): return action.id) == [&"defense_2", &"workforce_1", &"army_capacity_1", &"mining_1", &"cannon_range_1", &"recovery_1"], "completed attack track removes pagination and keeps all six remaining research tracks reachable")
	await _key(KEY_Q)
