extends SceneTree
## Authored templates plus primitive clones exercise full eight-player capacity
...
func _run() -> void:
	...
	for owner in range(8):
		...
		var unit_state: Dictionary = replication._entity_state(example, 7)
		...
		for building_index in range(20):
			...
			var state: Dictionary = replication._entity_state(building, 7)
			state.id = 2000 + states.size()
			if owner == 7 and kind in ["barracks", "factory", "headquarters"]:
			...
			states.append(state)
	var snapshot: Dictionary = replication.build_snapshot(7)
	snapshot.entities = states
	var cells := PackedByteArray()
	cells.resize(9216)
	cells.fill(2)
	snapshot.fog = {"owner_id": 7, "alliance_id": 7, "map_size": [192.0, 192.0], "width": 96, "height": 96, "revision": 1,
		"cells": Marshalls.raw_to_base64(cells), "revealed_building_alliances": [1, 1, 1, 1, 1, 1, 1, 1]}
	check(states.size() == 1056, "896_units_and_160_production_and_defense_buildings")
	check(states.filter(func(state): return state.has("plan")).size() == 112, "all_112_owner_units_have_full_nine_entry_plans_only")
	check(replication._valid_snapshot(snapshot), "eight_owner_snapshot_fields_match_actual_replication_schema")
	...
	var encoded := NetworkProtocol.encode_snapshot(snapshot, 7, 1, "1".repeat(32))
	...
	check(decoded.get("payload", {}).get("entities", []).size() == 1056, "all_eight_owner_entities_survive_codec")
	check(NetworkProtocol.decoded_size(encoded) < NetworkProtocol.MAX_PACKET_BYTES, "decompressed_payload_below_hard_limit")
	if not decoded.is_empty():
		check(replication._valid_snapshot(decoded.payload), "decoded_eight_owner_schema_still_valid")
		...
		check(decoded.payload.players.all(func(player): return player.has("private") == (int(player.owner_id) == 7)), "only_owner_seven_private_economy")
		check(decoded.payload.players[7].private.army_capacity_level == 2 and decoded.payload.players[7].private.mining_level == 3,
			"expanded_army_and_mining_levels_roundtrip_in_owner_private_state")
		check(decoded.payload.entities.all(func(state): return int(state.owner) == 7 or (not state.has("plan") and not state.has("production"))),
			"other_players_plans_and_production_stay_private_at_full_capacity")
		var invalid: Dictionary = decoded.payload.duplicate(true)
		invalid.players[7].alliance_id = 4
		check(not replication._valid_snapshot(invalid), "replica_cannot_change_room_alliance")
		invalid = decoded.payload.duplicate(true)
		invalid.players[7].eliminated = 1
		check(not replication._valid_snapshot(invalid), "elimination_requires_boolean")
		invalid = decoded.payload.duplicate(true)
		invalid.players[7].owner_id = 8
		check(not replication._valid_snapshot(invalid), "owner_eight_outside_dense_roster_rejected")
	print("NETWORK_SIX_CODEC_RESULTS " + JSON.stringify({"checks": checks, "failures": failures,
		"units": 896, "buildings": 160, "own_plans": 112, "plan_entries": UnitOrderPlan.MAX_ENTRIES, "fog_cells": 9216, "memories": 0,
