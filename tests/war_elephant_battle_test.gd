extends SceneTree
## Actual rendered combat, corpse lifecycle and 60/500-unit batch capacity.
...
func _run() -> void:
	...
	game.hud.hide()
	for i: int in 3:
		game.spawn_unit("war_elephant", 0, Vector3((i-1)*3.0, 0, -2.8))
		game.spawn_unit("spearman", 1, Vector3((i-1)*3.0, 0, 2.8))
	game.set_running(true)
	await create_timer(3.2).timeout
	check(game.owned_entities(0,"units").any(func(u: BattleUnit): return u.hp < u.max_hp), "war elephants take real combat damage")
	check(game.owned_entities(1,"units").any(func(u: BattleUnit): return u.hp < u.max_hp), "war elephant attacks damage enemy infantry")
	await capture("combat")
	await clear_armies()
	var corpse: BattleUnit = game.spawn_unit("war_elephant", 0, Vector3.ZERO)
	var corpse_id: int = corpse.get_instance_id()
	game.camera_rig.camera.size = 6
	game.set_running(true)
	corpse.receive_damage(360)
	...
	check(game.get_node("UnitRenderBatches").registered_models == 0, "death releases all elephant batch slots")
	Engine.max_fps = 0
	DisplayServer.window_set_vsync_mode(DisplayServer.VSYNC_DISABLED)
	for total: int in [60, 500]:
		game.set_running(false)
		var elephant_count: int = 10 if total == 60 else 250
		var army_size: int = total-elephant_count
		for i: int in army_size:
			var row: int = i / 20
			var x: float = (i % 20 - 9.5) * 2.5
			if i < elephant_count:
				game.spawn_unit("war_elephant", 0, Vector3(x, 0, -2.5-row*2.5))
			game.spawn_unit("spearman" if i % 2 == 0 else "archer", 1, Vector3(x, 0, 2.5+row*2.5))
		...
		while Time.get_ticks_usec() < deadline:
			...
			previous = now
		frames.sort()
		samples.append({"initial_units":total,"elephants":elephant_count,"remaining_units":game.sandbox_unit_count,"frame_median_ms":frames[frames.size()/2],"frame_p95_ms":frames[int(frames.size()*.95)]})
	...
	print("ELEPHANT_BATTLE ",checks," checks; ",failures.size()," failures; ",JSON.stringify(samples))
