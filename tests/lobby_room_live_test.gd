...
func _run() -> void:
	...
	var difficulty: OptionButton = lobby.get_node("%Slots").get_child(2).get_node("Difficulty")
	difficulty.select(2)
	difficulty.item_selected.emit(2)
	check(await until(func(): return host.room.slots[2].bot_difficulty == "very_hard" and guest.room.slots[2].bot_difficulty == "very_hard" and not host.room.slots[1].ready), "difficulty_is_broadcast_to_guest_and_clears_readiness")
	check(lobby._pending_slots.is_empty() and difficulty.selected == 2 and difficulty.tooltip_text.contains("3 倍"), "native_difficulty_waits_for_acknowledgement_and_displays_yield")
	check(lobby.get_node("%StartMatch").disabled, "difficulty_change_requires_human_to_prepare_again")
	guest.set_ready(true)
	check(await until(func(): return host.room.slots[1].ready), "guest_can_prepare_after_difficulty_change")
	...
	if not starts.is_empty():
		...
		check(starts[0].players[2].bot_difficulty == "very_hard" and starts[0].players[3].bot_difficulty == "normal", "different_computer_difficulties_survive_authoritative_match_start")
