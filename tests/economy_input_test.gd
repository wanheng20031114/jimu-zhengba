...
func _construction_case(worker: Node3D, second: Node3D, mine: Node3D) -> void:
	...
	_check(sites.size() == 1 and game.gold == before_gold - 150, "native Shift click creates one foundation and reserves one-hundred-fifty gold")
	if sites.size() != 1:
		return
	...
	sites = _sites()
	_check(sites.size() == 2 and game.gold == before_gold - 300, "second Shift placement creates and charges a distinct tower")
	if sites.size() != 2:
		return
	...
	_check(not queued.alive and game.gold == before_gold - 150, "native Delete cancels the untouched queued foundation and refunds one-hundred-fifty gold")
	await _key(KEY_DELETE)
	_check(game.gold == before_gold - 150, "repeated Delete cannot refund a canceled foundation twice")
	game.select_entities([worker])
	_freeze(worker, false)
