...
func _run() -> void:
	...
	await physics_frame
	await physics_frame
	var cannon: BattleUnit = game.spawn_unit("triple_cannon",0,Vector3(0,0,-11))
	...
	check(cannon.position.distance_to(at) < .03,"held two-wheel chassis remains still")
	...
	check(target.hp == hp-8,"real cannonball deals exactly 8 to ten-armor building")
	cannon.stop()
	check(cannon.position.distance_to(target.position) > 3 and cannon.position.is_finite(),"wall-side shot retains valid standoff position")
	await game.prepare_shutdown()
	game.queue_free()
	await process_frame
	await process_frame
	FileAccess.open("res://.local/triple-cannon-20260913/navigation-results.json",FileAccess.WRITE).store_string(JSON.stringify({"checks":checks,"failures":failures,"minimum_clearance":minimum_clearance},"\t"))
	print("TRIPLE_CANNON_NAVIGATION ",checks," checks; ",failures.size()," failures")
