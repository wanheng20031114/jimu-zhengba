...
## Six actual main scenes exercise their own authority, fog, commands and replicas.

var _received_wire: Dictionary = {}

func host_steps() -> void:
	if not await until(func(): return all_phase("world_ready"), 25.0):
		check(false, "all_six_native_worlds_ready")
		return
	...
	publish("orders")
	check(await until(func(): return all_phase("orders"), 20.0), "five_guests_submit_authoritative_commands")
	check(await until(func():
		for guest in range(1, peer_count):
			...
			if unit.global_position.distance_to(vec(starts[str(guest)])) <= 1.0:
				return false
		return true, 8.0), "all_five_remote_moves_execute")
	for guest in range(1, peer_count):
	...
	publish("recipient_five_drop")
	check(await until(func(): return game.bots.has(5), 18.0), "sixth_human_disconnect_becomes_bot")
	publish("recipient_five_resume")
	check(await until(func(): return not game.bots.has(5) and all_phase("recipient_five_resume"), 18.0), "sixth_human_reconnect_recovers_same_owner")
	check(game.get_instance_id() == scene_identity and match_starts == 1, "host_keeps_same_six_player_scene")
	...
	if int(NetworkProtocol.MODES[test_mode].teams) > 2:
		...
		publish("elimination")
		check(await until(func(): return all_phase("elimination"), 12.0), "all_six_clients_receive_public_elimination")
	publish("steady")
	check(await until(func(): return all_phase("steady"), 10.0), "all_six_clients_keep_receiving_states")
	await seconds(2.0)
	...
	while Time.get_ticks_msec() - started_msec < 150000:
		...
		match stage:
			"orders":
				...
				check(await until(func(): return game.entities_by_id.has(own_id), 12.0), "own_six_player_unit_replicated")
			...
			"recipient_five_drop":
				if owner == 5:
					drop_transport()
				phase = stage
			"recipient_five_resume":
				if owner == 5:
					var before := snapshots
					resume_transport()
					check(await until(func(): return relay.connection_state == "match" and snapshots > before + 1, 18.0), "owner_five_authenticates_and_receives_new_state")
					check(relay.owner_id == 5 and game.get_instance_id() == scene_identity and match_starts == 1, "owner_five_resume_keeps_identity_and_scene")
				phase = stage
			"elimination":
			...
			"finish":
				...
				check(snapshot_rejections == 0 and _snapshot_tick_regressions == 0, "six_owner_snapshot_schema_and_sequence")
				check(snapshots >= 20, "continuous_six_owner_snapshots")
				...
				return
		status(true)
	check(false, "six_player_scenario_timeout")
