...
func _run() -> void:
	...
	game.hud.hide()
	for i: int in 3:
		game.spawn_unit("light_cavalry", 0, Vector3((i-1)*3.0, 0, -2.8))
		game.spawn_unit("spearman", 1, Vector3((i-1)*3.0, 0, 2.8))
	game.set_running(true)
	await create_timer(3.2).timeout
	check(game.owned_entities(0,"units").any(func(u: BattleUnit): return u.hp < u.max_hp), "light cavalry take real combat damage")
	check(game.owned_entities(1,"units").any(func(u: BattleUnit): return u.hp < u.max_hp), "light cavalry attacks damage enemy infantry")
	await capture("combat")
	await clear_armies()
	var corpse: BattleUnit = game.spawn_unit("light_cavalry", 0, Vector3.ZERO)
	var corpse_id: int = corpse.get_instance_id()
	game.camera_rig.camera.size = 6
	game.set_running(true)
	corpse.receive_damage(90)
	...
	check(game.get_node("UnitRenderBatches").registered_models == 0, "death releases all cavalry batch slots")
	Engine.max_fps = 0
	DisplayServer.window_set_vsync_mode(DisplayServer.VSYNC_DISABLED)
	for total: int in [100, 500]:
		game.set_running(false)
		var cavalry_count: int = 50 if total == 100 else 250
		var army_size: int = total-cavalry_count
		for i: int in army_size:
			var row: int = i / 20
			var x: float = (i % 20 - 9.5) * 2.5
			if i < cavalry_count:
				game.spawn_unit("light_cavalry", 0, Vector3(x, 0, -2.5-row*2.5))
		...
		while Time.get_ticks_usec() < deadline:
			...
			previous = now
		frames.sort()
		samples.append({"initial_units":total,"cavalry":cavalry_count,"remaining_units":game.sandbox_unit_count,"frame_median_ms":frames[frames.size()/2],"frame_p95_ms":frames[int(frames.size()*.95)]})
	...
	print("LIGHT_CAVALRY_BATTLE ",checks," checks; ",failures.size()," failures; ",JSON.stringify(samples))
