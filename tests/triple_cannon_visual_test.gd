...
func _run() -> void:
	...
	check(triangles <= 7000,"triangle budget")
	check(codex.get_node("%Stats").text.contains("280") and codex.get_node("%Stats").text.contains("160"),"approved codex values")
	...
	codex._camera.position = Vector3(1.6,1.5,-5.5)
	codex._camera.look_at(Vector3(0,1,-.7),Vector3.UP)
	...
	for action: int in [1,2]:
		...
		for frame: int in 48 if action == 2 else 16:
	...
	await create_timer(.4).timeout
	await RenderingServer.frame_post_draw
	var bounds: Rect2i = portraits.portrait("triple_cannon").get_image().get_used_rect()
	print("PORTRAIT_BOUNDS ",bounds)
	portraits.portrait("triple_cannon").get_image().save_png(OUTPUT+"portrait.png")
	...
	game.hud.get_node("Sidebar/Scroll/Content/Kinds/triple_cannon").pressed.emit()
	check(game.paint_kind == "triple_cannon" and game._ghost.kind == "triple_cannon","sandbox saved placement preview")
	...
	game.camera_rig.camera.size = 13
	for entry: Array in [["cannon",-3.2],["triple_cannon",.2],["heavy_cannon",3.7]]:
		game.spawn_unit(entry[0],0,Vector3(entry[1],0,0))
	await capture("comparison",root)
	game.clear_units()
	await process_frame
	for owner: int in 3:
		game.spawn_unit("triple_cannon",owner,Vector3((owner-1)*3.2,0,1.6))
	...
	game.clear_units()
	await process_frame
	var corpse: BattleUnit = game.spawn_unit("triple_cannon",0,Vector3.ZERO)
	game.camera_rig.camera.size = 7
	game.set_running(true)
	corpse.receive_damage(160)
	await create_timer(.55).timeout
	await capture("death",root)
	check(not corpse.alive,"triple cannon uses ordinary death lifecycle")
	...
	print("TRIPLE_CANNON_VISUAL ",checks," checks; ",failures.size()," failures")
