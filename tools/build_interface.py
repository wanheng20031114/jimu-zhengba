...
label('GroupHint','Groups','Ctrl + 数字  编队',609,0,170,29,11,'Color(0.81,0.78,0.65,1)')
...
# Native saved navigation polygons; every cell shares its edges with its neighbors.
...
blocked=[(x,z,w,d) for _,_,_,x,z,w,d in buildings]
obstacle_path=ROOT/'assets/environment_obstacles.json'
if obstacle_path.exists():
    for entry in json.loads(obstacle_path.read_text(encoding='utf-8'))['obstacles']:
        p=entry['position']; sz=entry['size']; rot=entry.get('rotation_y',0)
        # Mesh uses world axis aligned bounds of the obstacle, enlarged for agents.
        w=abs(math.cos(rot))*sz[0]+abs(math.sin(rot))*sz[2]
        d=abs(math.sin(rot))*sz[0]+abs(math.cos(rot))*sz[2]
        blocked.append((p[0],p[2],w,d))
vertices=[]; index={}; polys=[]
def vert(x,z):
    key=(x,z)
    if key not in index:
        index[key]=len(vertices);vertices.append((x,0.03,z))
    return index[key]
for x in range(-40,40):
    for z in range(-40,40):
        px=x+.5;pz=z+.5
        if any(abs(px-bx)<w/2+1.15 and abs(pz-bz)<d/2+1.15 for bx,bz,w,d in blocked):continue
        polys.append([vert(x,z),vert(x,z+1),vert(x+1,z+1),vert(x+1,z)])
nav='[gd_resource type="NavigationMesh" format=3]\n\n[resource]\nvertices = PackedVector3Array('+', '.join(str(v) for point in vertices for v in point)+')\npolygons = Array[PackedInt32Array](['+', '.join('PackedInt32Array('+', '.join(map(str,p))+')' for p in polys)+'])\nagent_radius = 0.75\ncell_size = 0.25\ncell_height = 0.25\n'
(ROOT/'assets/battle_navigation.tres').write_text(nav,encoding='utf-8')
...
m.node('Sun','DirectionalLight3D','.',rotation_degrees='Vector3(-54,-31,0)',light_color='Color(1,0.89,0.72,1)',light_energy='1.15',shadow_enabled='true',directional_shadow_max_distance='135.0',shadow_bias='0.035',shadow_normal_bias='0.7',directional_shadow_blend_splits='true',directional_shadow_mode='2',light_angular_distance='1.6')
m.node('CameraRig','Node3D','.',position='Vector3(-10,0,14)',script='ExtResource("camera")')
m.node('Camera3D','Camera3D','CameraRig',position='Vector3(0,42,29.409)',rotation_degrees='Vector3(-55,0,0)',projection='1',size='32.0',near='0.1',far='220.0',current='true')
...
for i in range(5):starts.append(('swordsman',1,18+(i%3)*1.7,-15-(i//3)*1.5))
for i in range(4):starts.append(('archer',1,21+i*1.5,-18))
...
m.save('main.tscn')
print('Authored HUD, main scene and %d navigation polygons.' % len(polys))
