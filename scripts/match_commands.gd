...
func submit(command: Dictionary, owner: int) -> Dictionary:
	...
	if command.get("kind", "") not in KINDS or not command.get("seq", 0) is int:
		return failure("无效命令")
	var sequence: int = command.seq
...
func execute(command: Dictionary, owner: int) -> Dictionary:
	...
	for id: Variant in ids:
		if not (id is int or id is float):
	...
	if not (target_id is int or target_id is float):
	...
	if at.distance_squared_to(game.clamp_to_map(at)) > 0.01:
	...
	match kind:
		...
		"cancel_training":
			return target.get_node("Production").cancel_training(int(command.get("index", 0))) if own_building else failure("需要自己的生产建筑")
		"cancel_site":
		...
		"rally":
			...
			var mine: ResourceVein = game.entities_by_id.get(int(command.get("mine", 0))) as ResourceVein
