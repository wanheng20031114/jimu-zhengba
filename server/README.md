...
通道 0 用于可靠指令/房间，通道 1 用于可靠事件；房主到中继的有序不可靠快照按收件人使用通道 2–7，中继到单个客户端则使用通道 2。独立的序号窗口避免后发给另一玩家的快照淘汰本玩家乱序到达的快照。消息仅接受 JSON 基础值，向量须转成数组；不反序列化 Godot Object/Resource。每指令 4 KiB、每事件 32 KiB、每快照 512 KiB、嵌套 12 层、最多 65536 个基础值；100 指令/秒、每目标最多 20 快照/秒，附带总包数及总字节上限。快照允许 ENet 不可靠分片：丢片会丢弃旧快照，不阻塞可靠命令。游戏应优先发送紧凑字段和变化数据，512 KiB 是防护上限，不是带宽预算。客户端最多接收 1536 个可见单位／建筑实体，为六人各 100 军事人口和 12 农民之外的建筑、工地留出空间。
...
客户端以 120 毫秒缓冲平滑显示，位置线性插值、朝向走最短弧，攻击动作按房主发来的动画时间采样。原生 `AnimationPlayer.seek(..., update_only=true)` 不执行方法或音频轨；单位物理、避让、攻击计时器和生产逻辑均停用。客户端不预测伤害和经济，断包时停在最后已知位置。实体离开视野时静默释放，由迷雾层显示已见建筑轮廓；此操作不会调用阵亡、退款或死亡音效。可靠弹道、命中和音效由游戏事件路径单独处理。
...
部署脚本只负责新服务，不更改云安全组、不停旧服务、不安装整套 Web 服务。ENet 中继的全部游戏流量经过服务器，实际并发需按服务器公网带宽和压测结果确定。
...
`python tests/network_runner.py six-codec` 用原生场景作为模板，构建 672 单位、120 建筑、9216 迷雾格、本方 112 个单位各 9 项可见指令计划及满生产队列的 primitive 快照，验证 512 KiB/65536 值预算、owner5 私有科技及联盟不可变／淘汰布尔字段约束；敌方建筑记忆已移除。测试不生成 672 个单位场景，不把合成压缩字节当作实测网络吞吐。
